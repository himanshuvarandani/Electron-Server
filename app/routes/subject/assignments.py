...
async def post_subject_assignments(
    subject_id, body: AssignmentRequestBody, response: Response, Auth: AuthJWT = Depends()
):
    Auth.jwt_required()
...
async def get_assignment_details(
    ...
    assignment = (
        db.query(Assignments).filter(Assignments.id == assignment_id).first()
    )
...
async def get_assignment_marks(
    ...
    marks = (
        db.query(Marks).filter(Marks.assignment_id == assignment_id).all()
    )
