...
async def teacher_class_subjects(teacher_id, class_id: int, response: Response):
  subjects = db.query(Subjects).join(TeacherSubjectMap, TeacherSubjectMap.teacher_id==teacher_id).join(SubjectClassMap, SubjectClassMap.class_id==class_id).filter(TeacherSubjectMap.subject_id==SubjectClassMap.subject_id).filter(Subjects.id==TeacherSubjectMap.subject_id).all()
